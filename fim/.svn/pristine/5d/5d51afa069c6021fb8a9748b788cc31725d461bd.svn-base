...
const int kBackgroundSkinImageCount = 6;


CMainFrame::CMainFrame( LPCTSTR pszXMLPath )
	: CXMLWnd(pszXMLPath)
{
...
void CMainFrame::SetBkColor(DWORD dwBackColor)
{
	CControlUI* background = m_PaintManager.FindControl(kBackgroundControlName);
	if (background != NULL)
	{
		background->SetBkImage(_T("bg0.png"));
...
void CMainFrame::UpdateFriendsList()
{
	...
	if (pFriendsList != NULL)
	{
		...
		item.id = _T("2");
		item.folder = false;
		item.logo = _T("./images/default.png");
		...
		item.id = _T("4");
		item.folder = false;
		item.logo = _T("./images/icon_home.png");
...
void CMainFrame::UpdateGroupsList()
{
	CGroupsUI* pGroupsList = static_cast<CGroupsUI*>(m_PaintManager.FindControl(kGroupsListControlName));
	if (pGroupsList != NULL)
	{
...
void CMainFrame::UpdateMicroBlogList()
{
